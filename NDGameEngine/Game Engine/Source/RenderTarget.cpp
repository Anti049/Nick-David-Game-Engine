...
void RenderSurface::Create(unsigned int unWidth, unsigned int unHeight, DXGI_FORMAT eFormat, std::string szName)
{
	...
	RendererType::m_pDevice->CreateTexture2D(&ppTexDesc, NULL, &m_pTexture2D);
	// Render Target
	ppRTVDesc.Format = ppTexDesc.Format;
	ppRTVDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
	ppRTVDesc.Texture2D.MipSlice = 0;
	RendererType::m_pDevice->CreateRenderTargetView(m_pTexture2D, &ppRTVDesc, &m_pTarget);
	...
	RendererType::m_pDevice->CreateShaderResourceView(m_pTexture2D, &ppSRVDesc, &m_pShaderResourceView);
...
void DepthSurface::Create(unsigned int unWidth, unsigned int unHeight, DXGI_FORMAT eFormat, std::string szName)
{
	...
	RendererType::m_pDevice->CreateTexture2D(&desc, NULL, &m_pTexture2D);
	if (flags & D3D11_BIND_DEPTH_STENCIL)
	{
		...
		RendererType::m_pDevice->CreateDepthStencilView(m_pTexture2D, &depthStencilDesc, &m_pDSV);
	}
	if (flags & D3D11_BIND_SHADER_RESOURCE)
	{
		...
		RendererType::m_pDevice->CreateShaderResourceView(m_pTexture2D, &shaderResourceDesc, &m_pShaderResourceView);
	}
...
void RenderTarget::Initialize(std::string& szName, int nWidth, int nHeight, Vector4 vClearColor, unsigned int unNumTargets)
{
	...
	RendererType::m_pDevice->CreateTexture2D(&tex2dDesc, NULL, &m_pDepthTexture);
	D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc;
	ZeroMemory(&dsvDesc, sizeof(dsvDesc));
	dsvDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
	dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
	RendererType::m_pDevice->CreateDepthStencilView(m_pDepthTexture, &dsvDesc, &m_pDepthStencilView);
...
void RenderTarget::InitializeWithBackbuffer(std::string& szName, int nWidth, int nHeight, ID3D11Texture2D* pTexture, Vector4 vClearColor)
{
	...
	RendererType::m_pDevice->CreateRenderTargetView(pTarget->GetTexture2D(), NULL, &pRTV);
	pTarget->SetRTV(pRTV);
...
void RenderTarget::ActivateTarget(bool bClear, unsigned int unClearFlag)
{
	...
	RendererType::BeginEvent(0, szEvent.c_str());
	{
		RendererType::m_pImmediateContext->RSSetViewports(1, &m_sViewPort);
		ID3D11ShaderResourceView* pNullSRV[] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
		RendererType::m_pImmediateContext->PSSetShaderResources(0, ArraySize(pNullSRV), pNullSRV);

		ID3D11RenderTargetView* pRTVs[8];
		for (unsigned int i = 0; i < m_unNumRenderTargets; i++)
			pRTVs[i] = m_pRenderTargets[i]->GetRTV();
		RendererType::m_pImmediateContext->OMSetRenderTargets(m_unNumRenderTargets, pRTVs, m_pDepthStencilView);
		...
		if (bClear)
		{
			...
			for (unsigned int i = 0; i < m_unNumRenderTargets; i++)
				RendererType::m_pImmediateContext->ClearRenderTargetView(m_pRenderTargets[i]->GetRTV(), fColor);
			RendererType::m_pImmediateContext->ClearDepthStencilView(m_pDepthStencilView, unClearFlag, 1.0f, NULL);
		}
	}
...
void RenderTarget::DeactivateTarget(void)
{
	...
	RendererType::BeginEvent(0, szEvent.c_str());
	{
		...
		RendererType::m_pImmediateContext->PSSetShaderResources(0, 1, pNullSRV);
		RendererType::m_pImmediateContext->OMSetRenderTargets(m_unNumRenderTargets, pNullRTV, NULL);
		RendererType::m_pImmediateContext->RSSetState(NULL);
	}
	RendererType::EndEvent();
}
